...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    s3_client.upload_file(subtitle_output_path, "ai-podcast-clipper-bucket", output_s3_key)
...
class AiPodcastClipper:
    ...
    def transcribe_video(self,base_dir:str, video_path:str)->str:
        ...
        print("Transcribing audio with whisper x...")
        ...
        print("Audio transcribed in "+ str(duration) +" seconds")
    ...
    def process_video(self,request: ProcessVideoRequest,token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        s3_client = boto3.client("s3")
        s3_client.download_file("ai-podcast-clipper-bucket", s3_key, str(video_path))
        
        print(os.listdir(base_dir))
        
        #1. Trarnscript video
        transcriprt_segments_json=self.transcribe_video(base_dir,video_path)
        transcriprt_segments=json.loads(transcriprt_segments_json)
        
        #2. Identify moments 
        print("Identifying clip moments...")
        inditified_moments_raw=self.identify_moments(transcriprt_segments)
        
        cleaned_json_string=inditified_moments_raw.strip()
        ...
        for index,moment in enumerate(clip_moments[:1]):
            if "start" in moment and "end" in moment:
                ...
                process_clip(base_dir,video_path,s3_key,moment["start"],moment["end"],index,transcriprt_segments)
            
        if base_dir.exists():
            print(f"Cleaning temp dir after {base_dir}")
